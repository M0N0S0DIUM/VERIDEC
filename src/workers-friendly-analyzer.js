...
/**
 * Lightweight code analyzer for Cloudflare Workers compatibility
 * Provides basic JavaScript/TypeScript code analysis without heavy dependencies
 */
...
/**
 * Simple tokenizer for JavaScript code analysis
...
function tokenize(code) {
  ...
  while (i < code.length) {
    ...
    const commentMatch = code.slice(i).match(/^\/\/[^\n]*\n|\/\*[\s\S]*?\*\//);
    if (commentMatch) {
    ...
    for (const pattern of patterns) {
      const tokenMatch = code.slice(i).match(pattern.regex);
      if (tokenMatch) {
        tokens.push({
          type: pattern.type,
          value: tokenMatch[0],
    ...
    // If no pattern matched, advance by one character
    if (!matched) {
      tokens.push({
        type: 'unknown',
        value: code[i],
